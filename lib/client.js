  ...
  factory: (require) => {
    ...
    const DICTS = {
      en: {
        ...
        'band.flat': 'Flat',
        'dot.peak': 'Peak (Beijing)',
        'dot.offPeak': 'Off-peak (Beijing)',
    ...
    const LANG_KEY = 'dsh-usage-tracker:lang'
    const LANGS = [
      { id: 'tr', label: 'TR' },
      { id: 'en', label: 'EN' },
      { id: 'zh', label: '中' },
    ...
    const langStore = {
      ...
      set(id) {
        if (id === this.lang) return
        this.lang = id
        try { localStorage.setItem(LANG_KEY, id) } catch { /* ignore */ }
        this.notify()
      },
      cycle() {
        const idx = LANGS.findIndex((l) => l.id === this.lang)
        this.set(LANGS[(idx + 1) % LANGS.length].id)
      },
      label() { return (LANGS.find((l) => l.id === this.lang) || LANGS[0]).label },
    }

    function T(key, params) {
      const dict = DICTS[langStore.lang] || DICTS.tr
    ...
    // Force a re-render when the plugin language changes.
    function useLangTick() {
      const [, force] = react.useState(0)
      react.useEffect(() => langStore.subscribe(() => force((v) => v + 1)), [])
    }
    ...
    function FooterButton(props) {
      ...
      const band = data && data.currentBand ? data.currentBand : null
      ...
      return el('button', {
        ...
        band ? el('span', { className: 'utt-dot ' + (band === 'peak' ? 'utt-dot-peak' : 'utt-dot-offPeak'), title: T('dot.' + band) }) : null,
    ...
    function OverlayPanel() {
      ...
      return el('div', { className: 'utt-ov' },
        el('div', { className: 'utt-ov-head' },
          ...
          el('div', { className: 'utt-ov-head-actions' },
            el('button', { className: 'utt-lang', onClick: () => langStore.cycle(), title: 'Language / Dil / 语言' }, langStore.label()),
    ...
    function SettingsPanel() {
      ...
      const head = el('div', { className: 'utt-head' },
        ...
        el('div', { className: 'utt-actions' },
          ...
          el('button', { className: 'utt-btn', onClick: () => langStore.cycle(), title: 'Language / Dil / 语言' }, langStore.label()),
        ),
      )
...
.utt-ov-head-actions { display: flex; align-items: center; gap: 6px; }
.utt-dot { width: 9px; height: 9px; border-radius: 50%; display: inline-block; flex: none; }
.utt-dot-peak { background: #3b82f6; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.18); }
.utt-dot-offPeak { background: #22c55e; box-shadow: 0 0 0 3px rgba(34, 197, 94, 0.18); }
.utt-lang { border: 1px solid var(--dsw-alias-border-l2); background: transparent; color: var(--dsw-alias-label-secondary); cursor: pointer; font-size: 11px; padding: 2px 8px; border-radius: 6px; font-family: inherit; line-height: 18px; }
.utt-lang:hover { color: var(--dsw-alias-label-primary); background: var(--dsw-alias-bg-layer-2); }
